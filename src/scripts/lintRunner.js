...
async function runLint() {
    console.log(`${color.blue}[${getTimestamp()}]${color.reset} [LINT] Starting ESLint${FIX_MODE ? ' with auto-fix' : ''}...`);
    ...
    console.log(`${color.blue}[${getTimestamp()}]${color.reset} [LINT] ${color.green}Files scanned  : ${totalFiles}${color.reset}`);
    console.log(`${color.blue}[${getTimestamp()}]${color.reset} [LINT] ${color.green}Files clean    : ${cleanFiles}${color.reset}`);

    if (filesWithProblems > 0) {
        console.log(`${color.blue}[${getTimestamp()}]${color.reset} [LINT] ${color.yellow}Files with issues: ${filesWithProblems}${color.reset}`);
    }

    if (totalErrors > 0) {
        console.log(`${color.blue}[${getTimestamp()}]${color.reset} [LINT] ${color.red}Errors         : ${totalErrors}${color.reset}`);
    } else {
        console.log(`${color.blue}[${getTimestamp()}]${color.reset} [LINT] ${color.green}Errors         : ${totalErrors}${color.reset}`);
    }

    if (totalWarnings > 0) {
        console.log(`${color.blue}[${getTimestamp()}]${color.reset} [LINT] ${color.yellow}Warnings       : ${totalWarnings}${color.reset}`);
    } else {
        console.log(`${color.blue}[${getTimestamp()}]${color.reset} [LINT] ${color.green}Warnings       : ${totalWarnings}${color.reset}`);
    }
