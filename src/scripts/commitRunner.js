...
const commit = async () => {
	...
	console.log(
		`${logInfo} Selected type: ${color.green}${selectedType.type}${color.reset} — ${color.torquise}${selectedType.description}${color.reset}`,
	);
	...
	const confirmation = await askQuestion(
		`${log} Confirm commit? (${color.green}yes${color.reset}/${color.red}no${color.reset}, default ${color.green}yes${color.reset}): `,
	);
	...
	exec(`git commit -m "${fullMessage}"`, (error, stdout, stderr) => {
		if (error) {
			...
			console.error(
				`${logError} ${color.yellow}Tip: make sure your files are staged with ${color.green}git add <file>${color.yellow} or ${color.green}git add .${color.reset}`,
			);
			return;
		}
