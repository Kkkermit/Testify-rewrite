...
module.exports = {
		...
		.addSubcommand((command) =>
			command
				...
				.addStringOption((option) =>
					option.setName("prefix").setDescription("The new prefix you want to set.").setRequired(true),
				),
		)
		.addSubcommand((command) =>
			command.setName("check").setDescription("Check the current prefix of the bot in your server."),
		)
		.addSubcommand((command) =>
			command.setName("reset").setDescription("Reset the prefix of the bot in your server back to the default."),
		)
		.addSubcommand((command) =>
			command
				.setName("enable")
				.setDescription("Enable the prefix system in your server.")
				.addBooleanOption((option) =>
					option.setName("enable").setDescription("Enable the prefix system in your server.").setRequired(true),
				)
				.addStringOption((option) =>
					option
						.setName("prefix")
						.setDescription(
							"The prefix you want to set for the bot in your server. Leave blank to set to default prefix",
						)
						.setRequired(false),
				),
		)
	...
	async execute(interaction, client) {
		...
		if (sub !== "enable" && sub !== "disable" && sub !== "check" && (!guildData || !guildData.Enabled)) {
			return interaction.reply({
				content:
					"You **cannot** use this command as the prefix system has not yet been enabled. To enable the prefix system run **`prefix enable`**",
				flags: MessageFlags.Ephemeral,
			});
		}
		...
		switch (sub) {
			case "change":
				try {
					...
					if (prefix.length > 4)
						return interaction.reply({
							content: "The prefix **cannot** be longer than 4 characters!",
							flags: MessageFlags.Ephemeral,
						});
			...
			case "reset":
				try {
					if (!guildData)
						return interaction.reply({
							content: `The prefix is already set to the **default!** ( \`${client.config.defaultPrefix}\` )`,
							flags: MessageFlags.Ephemeral,
						});

					await prefixSystem.findOneAndUpdate({ Guild: interaction.guild.id }, { Prefix: client.config.defaultPrefix });
				...
				try {
					...
					if (!enable) {
						return await interaction.reply({
							content:
								"The prefix system won't be enabled and data has not been saved. To enable the prefix system, please choose the option **`True`** when trying again. If you just wanted to change the prefix, please use the command **`prefix change <prefix>`**",
							flags: MessageFlags.Ephemeral,
						});
					}
					...
					if (guildData?.Enabled) {
						return await interaction.reply({
							content: "The prefix system is already **enabled** in this guild.",
							flags: MessageFlags.Ephemeral,
						});
					}
					...
					const embed3 = new EmbedBuilder()
						...
						.addFields({
							name: "Change Custom Prefix",
							value: `*To change the custom prefix, run \`/prefix change <prefix>\`*`,
						})
			...
			case "disable":
				try {
					if (!guildData?.Enabled) {
						return await interaction.reply({
							content: "The prefix system is already **disabled** in this guild.",
							flags: MessageFlags.Ephemeral,
						});
					}
					...
					await interaction.reply({
						content: "The prefix system has been **disabled**.",
						flags: MessageFlags.Ephemeral,
					});
				} catch (error) {
					console.error(error);
