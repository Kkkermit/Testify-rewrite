...
module.exports = {
	...
	async execute(interaction, client) {
		...
		const dmEmbed = new EmbedBuilder()
			...
			.setTitle(`> Ban command ${client.config.arrowEmoji}`)
			.setDescription(
		...
		if (userID.id === client.user.id)
			return interaction.reply({ content: `You cannot use the ban hammer on me!`, flags: MessageFlags.Ephemeral });

		await userID.send({ embeds: [dmEmbed] }).catch((err) => {
			return client.logs.error(
				"[BAN] Failed to DM user. This can happen when their DM's are off, or the user is a bot.",
			);
		});

		const ban = await guild.members.ban(userID, { reason: `${interaction.user.tag} - ${reason}` }).catch((err) => {
			client.logs.error(`[BAN_ERROR] Error with Ban command: ${err}`);
		});
		if (ban) {
			(await interaction.channel.sendTyping(), await interaction.reply({ embeds: [banEmbed] }));
		} else if (!ban) {
			interaction.reply({
				content: `Failed to ban **${userID.tag}** from **${guild.name}**`,
