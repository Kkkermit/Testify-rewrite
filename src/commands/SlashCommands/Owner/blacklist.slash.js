...
const { blacklistSystem } = require("@schemas");
const { SlashCategory } = require("@utils");

module.exports = {
	usableInDms: true,
	devOnly: true,
	category: SlashCategory.OWNER,
	...
	async execute(interaction, client) {
		...
		switch (sub) {
			case "add": {
				if (!client.config.developerIds.includes(interaction.user.id)) {
				...
				const blacklist = await blacklistSystem.findOne({ userId: addBlacklistUser });
				...
				await blacklistSystem.create({ userId: addBlacklistUser, reason: reasonOption });
			...
			case "remove": {
				...
				const userToRemove = await blacklistSystem.findOne({ userId: removeBlacklistUser });
				...
				try {
					await blacklistSystem.findOneAndDelete({ userId: removeBlacklistUser });
