...
function checkDmUsability(command, interaction) {
	...
	if (!interaction.guild && command.usableInDms !== true) {
		interaction.reply({
			content: `\`${command.data.name}\` **cannot** be used in direct messages. Please navigate to a server to use this command.`,
...
function checkMessageDmUsability(command, message) {
	...
	if (!message.guild && command.usableInDms !== true) {
		message.reply(
			`\`${command.name}\` **cannot** be used in direct messages. Please navigate to a server to use this command.`,
		);
		return false;
	}
