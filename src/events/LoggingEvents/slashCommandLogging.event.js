			...
			try {
				...
				if (!channelId)
					return client.logs.error(
						`[SLASH_COMMAND_LOGGING_ERROR] No channel ID provided in the config. Please provide a valid channel ID.`,
					);
				...
				client.logs.error(
					`[SLASH_COMMAND_USED] Error while logging command usage. Check if you have the correct channel ID in your config.`,
				);
			}
		}
	},
