...
const { createCommandPages, getCategoryEmoji, getImages } = require("@utils");
...
module.exports = {
	...
	async execute(interaction, client) {
		...
		if (interaction.customId === "help_category_select") {
			...
			if (selectedCategory === "helpcenter") {
				...
				const embed = new EmbedBuilder()
					...
					.addFields({
						...
						value: `> The prefix for this server is \`${helpData.guildPrefix}\``,
					})
			...
			const isSlash =
				interaction.message.components[0]?.components[0]?.customId === "help_category_select" ||
			...
			const embed = new EmbedBuilder()
				...
				.setFooter({
					text: `${client.user.username} Help • ${selectedCategory} • Page 1/${pages.length}`,
		...
		if (interaction.customId === "help_category_select_prefix") {
			...
			const navigationRow = new ActionRowBuilder().addComponents(
				new ButtonBuilder().setCustomId(`help_back_prefix`).setLabel("Back to Help Menu").setStyle(ButtonStyle.Success),
				new ButtonBuilder()
		...
		if (interaction.customId.startsWith("help_page_")) {
			...
			const embed = new EmbedBuilder()
				...
				.setFooter({
					text: `${client.user.username} Help • ${category} • Page ${page + 1}/${pages.length}`,
					iconURL: client.user.displayAvatarURL(),
				});
		...
		if (interaction.customId.startsWith("help_switch_")) {
			...
			const commandsInCategory =
				newType === "slash" ? helpData.slashCommandCategories[category] : helpData.prefixCommandCategories[category];

			if (!commandsInCategory || commandsInCategory.length === 0) {
				return interaction.reply({
					content: `No ${newType} commands found in the ${category} category`,
